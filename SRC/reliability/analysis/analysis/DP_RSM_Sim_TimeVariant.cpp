...
//////////////////////////////////////////////////////////////////////
// command: runDP_RSM_SimTimeVariantAnalysis -designPt dp.out  -output results.out  -ndir $n <-experimentalPointRule Uniform -gridInfo {-1  minY  maxY nPts 0  minY  maxY nPts0 1 minX1  maxX1 nPts1 2 minX2  maxX2 nPts2 ...}> 
					...
					:ReliabilityAnalysis()
{
	...
	ImpulseInterval = pImpulseInterval;
	...
	*/
	...
	for(int i = 0;i< numOfGridPlanes; i++) theGridPlanes[i]=0;
...
// ----
	if (strcmp(typeSurfaceDesign, "UnivariateDecomposition")==0){
		theSurfaceDesign = new UnivariateDecomposition(numOfPrincipalAxes+1, 0, true); // note here numOfPrincipalAxes+1 since dp direction is 1st axis
	}
	else if (strcmp(typeSurfaceDesign, "BivariateDecomposition")==0){
		theSurfaceDesign = new BivariateDecomposition(numOfPrincipalAxes+1, 0, 0 , true); // note here numOfPrincipalAxes+1 since dp direction is 1st axis
	}
	else {
...
// -----
	...
	if (strcmp(typeRespSurfaceSimulation, "ImportanceSampling")==0){
		theRespSurfaceSimulation = new RespSurfaceSimulation(
						...
						true,
						pLittleDt/pImpulseInterval,
						/*theHessian->getNormOfGradientU()*/ 0.0);
...
DP_RSM_Sim_TimeVariant::~DP_RSM_Sim_TimeVariant()
{

}

int DP_RSM_Sim_TimeVariant::analyze()
{
  //1  form Hessian;
	...
	for(  ii=0; ii<numOfPrincipalAxes+1; ii++) {  
		...
		if (ii==0){  // dp direction
			...
			for(int j=0; j<numOfGrid; j++){
				...
				if(theGridPlanes[ii]->isFEConverged()){ // convergence
					thePrincipalAxes[ii]->setValueOnAxis(j,gFunValue);
				    gFunValue2 =  theGridPlanes[ii]->getValueG2OnGrid(coord, 0.0, gFunValue, littleDt);   // add:1 
					thePrincipalAxes[ii]->setValueG2OnAxis(j,gFunValue2);                                 // add:2
					...
					if ((j==0) ||(j==1)){
						...
						double g0_2= thePrincipalAxes[ii]->getValueG2OnAxis(j-2);   //add
						...
						double g1_2 = thePrincipalAxes[ii]->getValueG2OnAxis(j-1);   //add

						gFunValue = (g1-g0)/(x1-x0)*(x-x1)+g1;
						gFunValue2 = (g1_2-g0_2)/(x1-x0)*(x-x1)+g1_2;   //add

						if (gFunValue>=0) { 
							gFunValue2 = -1.0e-14 + gFunValue2 -gFunValue;   //add
							gFunValue = -1.0e-14;
						}

						thePrincipalAxes[ii]->setValueOnAxis(j,gFunValue);
						thePrincipalAxes[ii]->setValueG2OnAxis(j,gFunValue2);   //add
					}
				...
				opserr<<"jth value  is: gFunValue= "<<gFunValue<<",gFunValue2="<<gFunValue2<<"  \n";
				file_coeff<<"jth value  is: "<<gFunValue<<",   "<<gFunValue2<<" \n";
			...
			for(j=startPt; j<numOfGrid; j++){
				...
				gFunValue2 = theGridPlanes[ii-1]->getValueG2OnGrid(0, coord,gFunValue, littleDt); //add
				...
				if(theGridPlanes[ii-1]->isFEConverged()){
					thePrincipalAxes[ii]->setValueOnAxis(j,gFunValue);
					thePrincipalAxes[ii]->setValueG2OnAxis(j,gFunValue2);
					...
					if ((j==startPt) ||(j==startPt+1)){
						...
						gFunValue2 = (g1_2-g0_2)/(y1-y0)*(y-y1)+g1_2;   //add
			...
			for( j=startPt-1; j>=0; j--){
				...
				if(theGridPlanes[ii-1]->isFEConverged()){
					thePrincipalAxes[ii]->setValueOnAxis(j,gFunValue);
					gFunValue2 = theGridPlanes[ii-1]->getValueG2OnGrid(0, coord,gFunValue, littleDt);
					thePrincipalAxes[ii]->setValueG2OnAxis(j,gFunValue2);  //add
		...
		for(  ii=0; ii<numOfGridPlanes; ii++) {  //for each plane
			...
			for(jj=0; jj< numOfGridXPt; jj++){
				for(int kk=0; kk< numOfGridYPt; kk++){
					...
					if (fabs(x)<1.0e-14) {
						...
						gFunValue2 = axis_2->getValueG2OnAxis(kk);
						
					}
					else if (fabs(y)<1.0e-14) {
						gFunValue = axis_1->getValueOnAxis(jj);
						gFunValue2 = axis_1->getValueG2OnAxis(jj);
					}
					else{
						gFunValue = theGridPlanes[ii]->getValueOnGrid(x,y);
						gFunValue2 = theGridPlanes[ii]->getValueG2OnGrid(x,y,gFunValue,littleDt);						
					...
					theGridPlanes[ii]->setGridValueG2(jj,kk,gFunValue2);
						
					output_v <<"G:  jj="<<jj<<", kk="<<kk<<",x="<<x<<", y="<<y<<", value = "<< gFunValue<<endln;			
					output_v <<"G2: jj="<<jj<<", kk="<<kk<<",x="<<x<<", y="<<y<<", value = "<< gFunValue2<<endln;
					
					opserr<<"-----------------Before correction---------------\n";
					opserr<<"G:  Plain:"<<ii<<": "<<x<<","<<y<<","<<gFunValue<<"\n";	
					opserr<<"G2: Plain:"<<ii<<": "<<x<<","<<y<<","<<gFunValue2<<"\n";							
			...
			for( jj=0; jj< numOfGridXPt; jj++){
				for(int kk=0; kk< numOfGridYPt; kk++){
					...
					double z2 = theGridPlanes[ii]->getSavedValueG2OnGrid(jj,kk);
					output_vis<<x<<"   "<<y<<"   "<<z<<"   "<<z2<<"\n";
					opserr<<"Before correction\n";
					opserr<<"Plain:"<<ii<<": "<<x<<","<<y<<","<<z<<"  "<<z2<<"\n";	
			...
			if(sum-1<=m){
				...
				for(int i_y = startPt; i_y<= min; i_y++){ 
				   ...
				   if (fabs(theGridPlanes[ii]->getSavedValueOnGrid(startPtX-(sum-i_y), startPtY-i_y))>1.0e19) {
					   ...
					   newValue2 = FEDivergenceCorrection2(ii, startPtX, startPtY, -(sum-i_y), -i_y, m,n, 3); // (startPtX, startPtY, incr_i_x, incr_i_y, maxincr_i_x, maxincr_i_y, first)
					   theGridPlanes[ii]->setGridValueG2(startPtX-(sum-i_y), startPtY-i_y, newValue2);
					   opserr<<"G2 correcting: ["<<  -(sum-i_y)<<","<<-i_y<<"] to"<<newValue2<<endln;
					   output_v <<"G2 correction::::: jj="<<startPtX-(sum-i_y)<<", kk="<<startPtY-i_y<<", value2 = "<< newValue2<<endln;
				...
				for(int i_y = startPt; i_y<= min; i_y++){ 
				   ...
				   if (fabs(theGridPlanes[ii]->getSavedValueOnGrid(startPtX-(sum-i_y), startPtY-i_y))>1.0e19) {
					   ...
					   theGridPlanes[ii]->setGridValueG2(startPtX-(sum-i_y), startPtY-i_y, newValue2);
					   opserr<<"G2 correcting: ["<<  -(sum-i_y)<<","<<-i_y<<"] to"<<newValue2<<endln;
					   output_v <<"G2 correction::::: jj="<<startPtX-(sum-i_y)<<", kk="<<startPtY-i_y<<", value = "<< newValue2<<endln;
			...
			if(sum-1<=m){
				...
				for(int i_y = startPt; i_y<= min; i_y++){ 
				   ...
				   if (fabs(theGridPlanes[ii]->getSavedValueOnGrid(startPtX+(sum-i_y), startPtY-i_y))>1.0e19) {
					   ...
					   newValue2 = FEDivergenceCorrection2(ii, startPtX, startPtY, (sum-i_y), -i_y, m,n, 4); // (startPtX, startPtY, incr_i_x, incr_i_y, maxincr_i_x, maxincr_i_y, first)
					   theGridPlanes[ii]->setGridValueG2(startPtX+(sum-i_y), startPtY-i_y, newValue2);
					   opserr<<"G2 correcting: ["<<  (sum-i_y)<<","<<-i_y<<"] to"<<newValue2<<endln;
					   output_v <<"G2 correction::::: jj="<<startPtX+(sum-i_y)<<", kk="<<startPtY-i_y<<", value = "<< newValue2<<endln;
			...
			if(sum-1<=m){
				...
				for(int i_y = startPt; i_y<= min; i_y++){ 
				   ...
				   if (fabs(theGridPlanes[ii]->getSavedValueOnGrid(startPtX-(sum-i_y), startPtY+i_y))>1.0e19) {
					   ...
					   newValue2 = FEDivergenceCorrection2(ii, startPtX, startPtY, -(sum-i_y), i_y, m,n, 2); // (startPtX, startPtY, incr_i_x, incr_i_y, maxincr_i_x, maxincr_i_y, first)
					   theGridPlanes[ii]->setGridValueG2(startPtX-(sum-i_y), startPtY+i_y, newValue2);
					   opserr<<"G2  correcting: ["<<  -(sum-i_y)<<","<<i_y<<"] to"<<newValue2<<endln;
					   output_v <<"G2  correction::::: jj="<<startPtX-(sum-i_y)<<", kk="<<startPtY+i_y<<", value = "<< newValue2<<endln;
			...
			if(sum-1<=m){
				...
				for(int i_y = startPt; i_y<= min; i_y++){ 
				   ...
				   if (fabs(theGridPlanes[ii]->getSavedValueOnGrid(startPtX+sum-i_y, startPtY+i_y))>1.0e19) {
					   ...
					   newValue2 = FEDivergenceCorrection2(ii, startPtX, startPtY, sum-i_y, i_y, m,n, 1); // (startPtX, startPtY, incr_i_x, incr_i_y, maxincr_i_x, maxincr_i_y, first)
					   theGridPlanes[ii]->setGridValueG2(startPtX+sum-i_y, startPtY+i_y, newValue2);
					   opserr<<"G2  correcting: ["<<  (sum-i_y)<<","<<i_y<<"] to"<<newValue2<<endln;
					   output_v <<"G2  correction::::: jj="<<startPtX+(sum-i_y)<<", kk="<<startPtY+i_y<<", value = "<< newValue2<<endln;
				   }
...
void DP_RSM_Sim_TimeVariant::setGridInfo(Vector *gridInfo)
{
...
int DP_RSM_Sim_TimeVariant::getNumOfAxis(int Axis, int numGridPlane)
{
	int i =0; 
	int k=numGridPlane; 
...
int DP_RSM_Sim_TimeVariant::getNumOfGridPlane(int i, int j)
{
	int k =j;
	int n = numOfPrincipalAxes;
	while (i>0){
...
double DP_RSM_Sim_TimeVariant::FEDivergenceCorrection(int numGridPlane, int startPtX, int startPtY, int incr_i_x, int incr_i_y, int m, int n, int type)
{
...
//FEDivergenceCorrection(ii, startPtX, startPtY, sum-i_y, i_y, m,n, 1); // (startPtX, startPtY, incr_i_x, incr_i_y, maxincr_i_x, maxincr_i_y, first)
double DP_RSM_Sim_TimeVariant::FEDivergenceCorrection2(int numGridPlane, int startPtX, int startPtY, int incr_i_x, int incr_i_y, int m, int n, int type)
{
	...
	//		y = rule_2->getPointCoordinate(i_y);
			...
			v_x_0 = theGridPlanes[ii]->getSavedValueG2OnGrid(i_x-2, i_y);
			v_x_1 = theGridPlanes[ii]->getSavedValueG2OnGrid(i_x-1, i_y);
