...
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/reinforcedConcretePlaneStress/TclModelBuilderUniaxialMaterialCommand.cpp,v $
...
#include <ConcreteZ01.h>		// JZ
#include <ConcreteZ02.h>		// JZ
#include <SteelZ01.h>			// JZ
#include <ConcreteL01.h>		// AL
#include <TendonL01.h>			// AL
					...
					TCL_Char **argv, TclModelBuilder *theTclBuilder, Domain *theDomain)
{
	...
	//SteelZ01
	else if (strcmp(argv[1],"SteelZ01") == 0) {
	// Check that there is the minimum number of arguments
	if (argc < 7) {
	    opserr << "WARNING insufficient arguments\n";
	    printCommand(argc,argv);
	    opserr << "Want: uniaxialMaterial SteelZ01 tag? fy? E0? fpc? rou?";
	    opserr << " <ac? rc?>" << endln;	
	    return TCL_ERROR;
	}
	...
	// Read required SteelZ01 material parameters
	double fy, E, fpc, rou;
	
	if (Tcl_GetDouble(interp, argv[3], &fy) != TCL_OK) {
	    opserr << "WARNING invalid fy\n";
	    opserr << "uniaxialMaterial SteelZ01: " << tag << endln;
	    return TCL_ERROR;
	}
	...
	if (Tcl_GetDouble(interp, argv[5], &fpc) != TCL_OK) {
	    opserr << "WARNING invalid fpc\n";
	    opserr << "uniaxialMaterial SteelZ01: " << tag << endln;
	    return TCL_ERROR;
	}

	if (Tcl_GetDouble(interp, argv[6], &rou) != TCL_OK) {
	    opserr << "WARNING invalid rou\n";
	    opserr << "uniaxialMaterial SteelZ01: " << tag << endln;
	    return TCL_ERROR;
	}
	...
	if (argc > 7) {
	    if (argc < 9) {
		opserr << "WARNING insufficient number of parameters for ac and rc\n";
		opserr << "uniaxialMaterial SteelZ01: " << tag << endln;
		return TCL_ERROR;
	    }
	    
	    if (Tcl_GetDouble(interp, argv[7], &ac) != TCL_OK) {
		opserr << "WARNING invalid ac\n";
		opserr << "uniaxialMaterial SteelZ01: " << tag << endln;
		return TCL_ERROR;
	    }
	    
	    if (Tcl_GetDouble(interp, argv[8], &rc) != TCL_OK) {
		...
		// Parsing was successful, allocate the material
		theMaterial = new SteelZ01(tag, fy, E, fpc, rou, ac, rc);
	}
	else
		// Parsing was successful, allocate the material
		theMaterial = new SteelZ01(tag, fy, E, fpc, rou);
    }   
//End of steelZ01


	//TendonL01
	else if (strcmp(argv[1],"TendonL01") == 0) {
	// Check that there is the minimum number of arguments
	if (argc < 8) {
	    opserr << "WARNING insufficient arguments\n";
	    printCommand(argc,argv);
	    opserr << "Want: uniaxialMaterial SteelZ01 tag? fy? E0? fpc? rou? eps?";
	    opserr << " <ac? rc?>" << endln;	
	    return TCL_ERROR;
	}
	...
	// Read required SteelZ01 material parameters
	double fpy, Eps, fpu, rou, epsp;
	
	if (Tcl_GetDouble(interp, argv[3], &fpy) != TCL_OK) {
	    opserr << "WARNING invalid fpy\n";
	    opserr << "uniaxialMaterial TendonL01: " << tag << endln;
	    return TCL_ERROR;
	}
	
	if (Tcl_GetDouble(interp, argv[4], &Eps) != TCL_OK) {
	    opserr << "WARNING invalid Eps\n";
	    opserr << "uniaxialMaterial TendonL01: " << tag << endln;
	    return TCL_ERROR;
	}

	if (Tcl_GetDouble(interp, argv[5], &fpu) != TCL_OK) {
	    opserr << "WARNING invalid fpu\n";
	    opserr << "uniaxialMaterial TendonL01: " << tag << endln;
	    return TCL_ERROR;
	}
	...
	if (Tcl_GetDouble(interp, argv[7], &epsp) != TCL_OK) {
	    opserr << "WARNING invalid epsp\n";
	    opserr << "uniaxialMaterial TendonL01: " << tag << endln;
	    return TCL_ERROR;
	}

	// Read optional TendonL01 material parameters
	double ac, rc;
	...
	if (argc > 8) {
	    ...
	    if (Tcl_GetDouble(interp, argv[8], &ac) != TCL_OK) {
		opserr << "WARNING invalid ac\n";
		opserr << "uniaxialMaterial TendonL01: " << tag << endln;
		return TCL_ERROR;
	    }
	    
	    if (Tcl_GetDouble(interp, argv[9], &rc) != TCL_OK) {
		opserr << "WARNING invalid rc\n";
		opserr << "uniaxialMaterial TendonL01: " << tag << endln;
		return TCL_ERROR;
	    }
		...
		// Parsing was successful, allocate the material
		theMaterial = new TendonL01(tag, fpy, Eps, fpu, rou, epsp, ac, rc);
	}
	else
		// Parsing was successful, allocate the material
		theMaterial = new TendonL01(tag, fpy, Eps, fpu, rou, epsp);
    }   
//End of TendonL01
    ...
    *************************************************************** */
    ...
    // ********************************* for ConcreteZ01 part ********************************
    else if (strcmp(argv[1],"ConcreteZ01") == 0) {
	if (argc < 5) {
	    opserr << "WARNING insufficient arguments\n";
	    printCommand(argc,argv);
	...
	// Read required ConcreteZ01 material parameters
	double fpc, epsc0;

	if (Tcl_GetDouble(interp, argv[3], &fpc) != TCL_OK) {
	    opserr << "WARNING invalid fpc\n";
	    opserr << "ConcreteZ01 material: " << tag << endln;
	    return TCL_ERROR;
	}
	...
	// Parsing was successful, allocate the material
	theMaterial = new ConcreteZ01(tag, fpc, epsc0);
    }
    // ********************************* end for ConcreteZ01 part ********************************    


	// ********************************* for ConcreteZ02 part ********************************
    else if (strcmp(argv[1],"ConcreteZ02") == 0) {
	if (argc < 6) {
	    opserr << "WARNING insufficient arguments\n";
	    printCommand(argc,argv);
	    opserr << "Want: uniaxialMaterial ConcreteZ02 tag? fpc? epsc0? Ec?" << endln;
	    return TCL_ERROR;
	}
	...
	if (Tcl_GetDouble(interp, argv[5], &Ec) != TCL_OK) {
	    opserr << "WARNING invalid Ec\n";
	    opserr << "ConcreteZ02 material: " << tag << endln;
	    return TCL_ERROR;
	}

	// Parsing was successful, allocate the material
	theMaterial = new ConcreteZ02(tag, fpc, epsc0, Ec);
    }
    // ********************************* end for ConcreteZ02 part ******************************** 


	// ********************************* for ConcreteL01 part ********************************
    else if (strcmp(argv[1],"ConcreteL01") == 0) {
	if (argc < 5) {
	    opserr << "WARNING insufficient arguments\n";
	    printCommand(argc,argv);
	...
	if (Tcl_GetDouble(interp, argv[3], &fpc) != TCL_OK) {
	    opserr << "WARNING invalid fpc\n";
	    opserr << "ConcreteL01 material: " << tag << endln;
	    return TCL_ERROR;
	}
	...
	// Parsing was successful, allocate the material
	theMaterial = new ConcreteL01(tag, fpc, epsc0);
    }
    // ********************************* end for ConcreteL01 part ******************************** 
