...
%The first patch command in the fiber section definition generates a
%rectangular patch of concrete fibers over the confined region of the
%section. The concrete model used for this
%patch is given by the previously defined UniaxialMaterial with tag 1.
%The corner points of this patch are (-10.5, -6) and (10.5, 6) and
%there are 10 fibers along the y-direction and one fiber along the
%z-direction as bending is only about the z-axis. This confined region
%patch is shown in figure~\ref{rcsection1}.
...
%\epsfxsize=5.5in
%%\epsfysize=4.2in
%\epsffile{./fig_files/RCsection1.eps}}
%\end{center}
%\caption{Fiber discretization of confined region}
%\label{rcsection1}
%\end{figure}

%The next two commands in the section definition create patches over
%portions of the unconfined region. These patches cover the unconfined
%regions at the boundaries of the section width (top and botton).
%Each patch contains
%10 fibers along the y-direction and one fiber along the z-direction.
%The UniaxialMaterial with tag 2 is associated with these fibers.
%The points (-12, 6), (12, 7.5) and (-12, -7.5), (12,-6) are the
%corners of each patch. These patches are shown in figure~\ref{rcsection2}.
...
%\epsfxsize=5.5in
%%\epsfysize=4.2in
%\epsffile{./fig_files/RCsection2.eps}}
%\end{center}
%\caption{Fiber discretization of unconfined region: top and bottom}
%\label{rcsection2}
%\end{figure}

%To finish the discretization of the unconfined region, two patches
%at the boundaries of the section depth (left and right) are added.
%Each patch contains
%two fibers, for which the UniaxialMaterial is given by tag 2.
%The patch corners are (-12, -6), (-10.5, 6) and (10.5, -6), (12, 6).
%These patches are shown in figure~\ref{rcsection3}.
...
%\epsfxsize=5.5in
%%\epsfysize=4.2in
%\epsffile{./fig_files/RCsection3.eps}}
%\end{center}
%\caption{Fiber discretization of unconfined region: left and right}
%\label{rcsection3}
%\end{figure}

%The last step in the definition of the fiber section is to create
%the steel reinforcing layers. To this end, three layers are generated.
%Each layer is straight, defined by two points and a number of fibers.
%The UniaxialMaterial with tag 3 is associated with each fiber in
%all three layers, and the area of each fiber is given by the variable
%\$As. Each layer is defined along constant values of y.
%The first layer contains three fibers and is defined by the points
%(10.5, 6) and (10.5, -6). The second layer contains only two fibers
%defined at (0, 6) and (0,-6). The third layer contains three fibers
%which span from (-10.5, 6) to (-10.5, -6). These layers are shown
%in figure~\ref{rcsection4}. For planar problems where these is a
%single axis of bending, each layer may be lumped into a single
%fiber; one fiber for each distinct value of y.
...
#nDMaterial  J2Plasticity     1   66.6666666666666667  40.   5.0  10.0  2.575   0.0  0.0 
...
block2D $nx $ny 1 1 $Shell $eleArgs {
1   -20    0     0
2   -20    0    40
3    20    0    40
4    20    0     0
...
recorder Node loser.out  disp -time -node $mid -dof 2
recorder plot loser.out CenterNodeDisp 625 10 625 450 -columns 1 2
