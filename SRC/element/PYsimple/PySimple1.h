...
**              This module is just playing around to learn the system.
**
...
{
  public:
    PySimple1(int tag, int soilType, double pult, double y50, double drag,
		      double dashpot);
	PySimple1();
    ~PySimple1();
  ...
  private:
	...
	double yref;		// reference point for Near Field spring
	...
	double nd;			// exponent for hardening shape of drag spring
	double dashpot;     // dashpot on the far-field (elastic) component

	// Generated parameters or constants (not user input)
	double NFkrig;		// stiffness of the "rigid" portion of Near Field srping
...
//	double CyRate;		// Committed velocity
