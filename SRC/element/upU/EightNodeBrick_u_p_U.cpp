...
const Vector &EightNodeBrick_u_p_U::getResistingForceIncInertia ()
{
    ...
    for (i=0; i<Num_Nodes; i++) {
      ...
      if ( acc.Size() != Num_Dof ) {
        opserr << "EightNodeBrick_u_p_U::getResistingForceIncInertia matrix and vector sizes are incompatable\n";
        exit(-1);
      }
      for (j=0; j<Num_Dof; j++) {
        a[i*Num_Dof+j] = acc(j);
...
//======================================================================
Vector EightNodeBrick_u_p_U::getInForceS ()
{
...
//======================================================================
Vector EightNodeBrick_u_p_U::getInForceP ()
{
