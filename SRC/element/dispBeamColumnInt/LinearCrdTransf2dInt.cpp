...
// LinearCrdTransf2dInt class. LinearCrdTransf2dInt is a linear
...
int 
LinearCrdTransf2dInt::initialize(Node *nodeIPointer, Node *nodeJPointer)	
{       
   int error;
   ...
   {
      opserr << "\nLinearCrdTransf2dInt::initialize";
...
int 
LinearCrdTransf2dInt::computeElemtLengthAndOrient()
{
   // element projection
   static Vector dx(2);
   ...
   if (L == 0.0) 
   {
      opserr << "\nLinearCrdTransf2dInt::computeElemtLengthAndOrien: 0 length\n";
      return -2;  
   }
...
const Vector &
LinearCrdTransf2dInt::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
...
const Vector &
LinearCrdTransf2dInt::getGlobalResistingForceInt(const Vector &pb, const Vector &p0)	//LMS
{
	// transform resisting forces from the basic system to local coordinates
	static double pl[6];
...
LinearCrdTransf2dInt::getCopy(void)		
{
  ...
  theCopy = new LinearCrdTransf2dInt(this->getTag(), offsetI, offsetJ);
...
int 
LinearCrdTransf2dInt::sendSelf(int cTag, Channel &theChannel)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "LinearCrdTransf2dInt::sendSelf - failed to send Vector\n";
    return res;
  }
...
int 
LinearCrdTransf2dInt::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "LinearCrdTransf2dInt::recvSelf - failed to receive Vector\n";
    return res;
  }
...
void
LinearCrdTransf2dInt::Print(OPS_Stream &s, int flag)
{
