 ...
 X(0), vectX(0), vectB(0), Asize(0), Bsize(0), factored(false)
{
	...
	cerr << size <<", " << numSuperDiag << ", " << numSubDiag << ") \n";
...
BandGenLinSOE::setSize(Graph &theGraph)
{
    ...
    int solverOK = theSolvr->setSize();
    if (solverOK < 0) {
	cerr << "WARNING:BandGenLinSOE::setSize :";
...
BandGenLinSOE::addA(const Matrix &m, const ID &id, double fact)
{
	...
	cerr << "BandGenLinSOE::addA()	- Matrix and ID not of similar sizes\n";
	return -1;
    }
...
BandGenLinSOE::addB(const Vector &v, const ID &id, double fact)
{
    ...
    int idSize = id.Size();        
    if (idSize != v.Size() ) {
	cerr << "BandGenLinSOE::addB()	- Vector and ID not of similar sizes\n";
	return -1;
    }    
...
{
    if (vectX == 0) {
	cerr << "FATAL BandGenLinSOE::getX - vectX == 0!";
	exit(-1);
    }    
    return *vectX;
}
...
{
    if (vectB == 0) {
	cerr << "FATAL BandGenLinSOE::getB - vectB == 0!";
	exit(-1);
    }    
    return *vectB;
}
...
BandGenLinSOE::setBandGenSolver(BandGenLinSolver &newSolver)
{
	...
	if (solverOK < 0) {
	    cerr << "WARNING:BandGenLinSOE::setSolver :";
